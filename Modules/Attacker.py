...
class attacker:
	...
	def prepareDicts(self, wordlistPath, proxylistPath):
		...
		print(colors.OKCYAN + "[" + colors.OKBLUE + colors.BOLD + "i" + colors.ENDC + colors.OKCYAN + "]" + " " + colors.ENDC + colors.DARK_CYAN + "Loaded " + colors.BOLD + str(attacker.proxylist_len) + colors.ENDC + colors.DARK_CYAN+ " proxies from file" + colors.ENDC)

	def setup(self, method, custom_request, cookies, threadsCount):
	...
	def attack_thread(self, target, username, wordlist, proxylist, errIdentfier, username_parameter, password_parameter, request_template_raw, cookiesObj, threadNum):
			...
			if attacker.threads__pause:
				...
				if currentResponse__text.find(errIdentfier) == -1 and currentResponse__code < 400:
					...
					if attacker.badResponse_counter == 1 and currentPwdLine != 1 and not attacker.algDetection_reserve:
						...
						#possible_pwd = linecache.getline(wordlist, currentPwdLine - 1).strip()
	...
	def attack(self, target, username, wordlist, proxylist, errIdentfier, username_parameter, password_parameter):
		...
		try:
			try:
				...
				# cant read files
				print(colors.OKCYAN + "[" + colors.FAIL + colors.BOLD + "!" + colors.ENDC + colors.OKCYAN + "]" + " " + colors.ENDC + colors.FAIL + "Can't read wordlist ot proxylist, please make sure that the path is right" + colors.ENDC)
				return
				...
				if attacker.proxylist_len >= 5:
					...
					try:
						...
						for threadNum in range(1, attacker.threadsCount + 1):
							...
							newThread = threading.Thread(target = attacker.attack_thread, args = (0, target, username, wordlist, proxylist, errIdentfier, username_parameter, password_parameter, request_template_raw, cookiesObj, threadNum, ))
						...
						print(colors.OKCYAN + "[" + colors.OKBLUE + colors.BOLD + "i" + colors.ENDC + colors.OKCYAN + "]" + " " + colors.ENDC + colors.OKCYAN + "All threads has been terminated and cleanup is done" + colors.ENDC)
